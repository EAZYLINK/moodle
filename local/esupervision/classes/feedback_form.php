...
require_once("$CFG->libdir/formslib.php");
 class local_esupervision_v1_feedback_form extends moodleform {
    ...
    public function validation($data, $files) {
        ...
        if (empty($data['feedback_comments'])) {
            $errors['feedback_comments'] = get_string('error_required', 'local_esupervision_v1');
        }
        if ($data['feedback_comments']<10) {
            $errors['feedback_comments'] = get_string('error_min_length', 'local_esupervision_v1');
        }
        return $errors;
    }
 }
...
use local_esupervision\coursetemplateslistform;
 ...
 */
function local_esupervision_extend_navigation_category_settings($parentnode, $context) {
    $capabilities = array(
    ...
    if (has_capability('local/esupervision:view', $context)
            && has_all_capabilities($capabilities, $context)
    ) {
        $parentnode->add(
            get_string('addcourse', 'local_esupervision'),
            new moodle_url('/local/esupervision/index.php', array('cateid' => $context->instanceid)),
            navigation_node::TYPE_SETTING,
            null,
            null,
            new pix_icon('t/add', get_string('addcourse', 'local_esupervision'))
        );
    };
}
...
function get_template_list() {
    ...
    $namecategoryid = get_config('local_esupervision', 'namecategory');
...
function get_template_list_form($cateid = null) {
    ...
    $redirecturl = $CFG->wwwroot.'/local/esupervision/index.php?cateid=' . $cateid . '&step=2';
...
function get_template_categories_form($cid, $cateid = null) {
    ...
    $capabilities = array(
        ...
        'moodle/course:create',
        'moodle/site:approvecourse',
        'local/esupervision:view',
    );

    $cateid = (int) $cateid ?? 1;

    $redirecturl = $CFG->wwwroot . '/local/esupervision/index.php?cateid=' . $cateid . '&step=3&cid=' . $cid;
...
function get_template_setting_form($cid, $categoryid, $cateid = null) {
    ...
    $redirecturl = $CFG->wwwroot.'/local/esupervision/process.php?cateid=' . $cateid . '&cid='
        . $cid
        . '&sesskey='
        . sesskey();

    $returnurl   = $CFG->wwwroot.'/local/esupervision/index.php?cateid=' . $cateid . '&step=4';

    $output = '';
    $output .= '<script src="'.$CFG->wwwroot.'/local/esupervision/js/jquery-1.8.3.min.js"></script>';
    $output .= '<link rel="stylesheet" href="'.$CFG->wwwroot.'/local/esupervision/css/bootstrap-datetimepicker.css">';
    $output .= '<link rel="stylesheet" href="'.$CFG->wwwroot.'/local/esupervision/css/throbber.css">';
    $output .= '<script src="'.$CFG->wwwroot.'/local/esupervision/js/bootstrap-datetimepicker.js"></script>';
    $output .= '<script src="'.$CFG->wwwroot.'/local/esupervision/js/process.js"></script>';
    $output .= '<div id="esupervision_validation_error_message" data-validation-message="'
    ...
    $table->data[] = array(
        html_writer::nonempty_tag(
            'label',
            get_string('coursename', 'local_esupervision'),
            array(
                'for' => 'course_name',
    ...
    $table->data[] = array(
        html_writer::nonempty_tag(
            'label',
            get_string('courseshortname', 'local_esupervision'),
            array(
    ...
    if ($course->format == 'event') {
        ...
        $table->data[] = array(
            get_string('datetime', 'local_esupervision'),
            html_writer::empty_tag(
                'input',
                array(
                    'type' => 'text',
        ...
        $options .= "</select>";
        $table->data[] = array(get_string('location', 'local_esupervision'), $options);
    }
